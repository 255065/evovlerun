...
def build_server(token_verifier: TokenVerifier | None = None) -> FastMCP:
    ...
    mcp.tool(
        ...
        description=(
            ...
            "Use canonical session_type values: easy, long, tempo, threshold, "
            "intervals, vo2max, fartlek, hills, recovery, race, strength, "
            "cross_training, rest. Aliases such as easy_run, long_run, "
            "recovery_run, 'easy run', 'langtur', and 'restitution' are accepted "
            "and normalized, but canonical values are preferred. Sessions may use "
            "distance_m or distance_km. The save is all-or-nothing: if any session "
